...
온라인 설치는 QC 썸네일 처리에 필요한 Pillow를 현재 사용자 Python 환경에 설치 시도합니다. PEP 668 externally-managed Python(Homebrew, Debian/Ubuntu 기본 Python)처럼 자동 설치가 거부되는 환경에서는 경고 후 설치를 계속 진행하며, Pillow가 준비될 때까지 Vision-QC는 fail-closed로 닫혀 있습니다. `--offline` 설치는 네트워크 접근 없이 파일만 복사하고, 전역 launcher/manifest 등록은 `--register`를 명시할 때만 수행합니다.
