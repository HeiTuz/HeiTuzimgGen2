...
## Folder preparation and retention

`scripts/folder_batch_prepare.py` converts an accessible source directory into a production manifest. It inventories JPEG, PNG, and WebP inputs without following symlinks, junctions, or reparse points; encountering one fails closed. Preparation is capped at 500 supported files and 5 GiB total input, and each batch reference is capped at 512 MiB. It excludes an explicit output subtree only when that subtree sits under the source folder, preserves relative directory names, and never modifies source files. Preparation records live absolute source references, so keep sources available and unchanged through approval. Immediately before every live transport, the runner copies the approved bytes through an open source handle into a current-user-private managed snapshot, verifies the copied hash and size against manifest evidence, and passes only the snapshot paths to Codex. Changed bytes fail closed and the snapshots use the normal temporary retention lifecycle; output paths remain relative to the selected output root.

When `--output-root` is omitted, the manifest and outputs are staged under the operating system's canonical temporary directory in a marked `HeiTuzImgGen2` job. With an explicit persistent output root, the manifest is instead stored under `<output-root>/.heituzimggen2-manifests/` so it remains available for resume, approval verification, QC, and retry work. The default temporary retention window is 24 hours; `scripts/cleanup_temp_outputs.py` deletes only expired `single-*` and `folder-*` job directories whose ownership markers match the exact role-specific field and type schema. It rejects non-finite timing values, calculates expiry from the newest safe descendant activity without following symlinks, junctions, or reparse points, serializes cleaner processes with a root lock, and coordinates deletion with process-held activity locks. Cleanup claims and quarantine names are bound to the exact canonical job ID; valid pre-rename claims abandoned by a crash become recoverable after five minutes, and malformed or ambiguous claims remain untouched. It ignores stale batch lock files, preserves unknown children and symlinks, and refuses a symlink/reparse-point or non-empty unmarked root. On Windows it verifies the current-user owner before adopting or changing any existing root, then applies and rechecks a current-user/SYSTEM-only inheritable DACL on managed roots and jobs; on POSIX it verifies ownership and mode `0700`. Quarantine-name restoration after a deletion error is best-effort: already removed files cannot be reconstructed, so cleanup targets only expired managed temporary jobs. Run or schedule the cleaner explicitly; creating or dry-running a job does not trigger deletion. Explicit output roots must be outside the managed temporary root, are outside this lifecycle, and are never auto-deleted.

Folder preparation does not authorize live generation. Run the emitted manifest through the normal dry-run, pilot, approval, QC, and retry process below.
