...
생성 후보는 최종 전달 전에 Vision-QC로 검수합니다. 기본 모드 `auto`는 별도 검수 모델을 고정하지 않고 현재 호스트의 기본 Vision 모델을 사용합니다. Hermes에서는 `vision_analyze`가 `auxiliary.vision` 설정을 따라 실행되므로, 기본 Vision 모델을 갱신하면 ImgGen2 QC도 자동으로 같은 모델을 사용합니다. 설정은 설치 폴더의 `vision-qc.json`에 기록되며 자격 증명 값은 저장하지 않습니다.
...
`auto`는 설치 환경의 Gemini 키나 Codex 버전을 보고 검수 모델을 고르지 않습니다. 이미지 생성 후 현재 호스트의 기본 Vision tool에 검수를 맡깁니다. `off`는 QC를 생략하는 대신 결과를 최종 통과본으로 승인하지 않습니다.
